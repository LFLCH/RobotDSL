...
<p align="center">
  <img src="assets/canvas_drawing_wrong.png" alt="ino valid compilation" width="300"><br>
</p>

2. PERFORMANCES: the interpreter is slow. For instance loops of 100 are enough to represent a few seconds for loading the result. 

3. GRAMMAR: The most obvious problem. We currently use an altered ast file from the one generated by Langium using the grammar. If we success to convert the abstract nodes into ts ```interface``` by changing the grammar, it would be great. The solution we found should only be temporary.    
